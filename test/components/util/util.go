...
func PortForward(t *testing.T, ns, pod string, ports []string) (stopChan chan struct{}, forwardedPorts []portforward.ForwardedPort) {
	...
	serverURL.Path = fmt.Sprintf("/api/v1/namespaces/%s/pods/%s/portforward", ns, pod)
	...
	case <-time.After(30 * time.Second):
		t.Fatalf("Timed out waiting for forwarding")
	}

	p, err := forwarder.GetPorts()
	if err != nil {
		t.Fatalf("Failed getting forwarded ports: %v", err)
	}
