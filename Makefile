...
.PHONY: lint
lint:
	golangci-lint run --enable-all --disable=godox --max-same-issues=0 --max-issues-per-linter=0 --build-tags aws,packet,e2e,disruptive-e2e,prometheus --new-from-rev=$$(git merge-base $$(cat .git/resource/base_sha 2>/dev/null || echo "master") HEAD) --modules-download-mode=$(MOD) --timeout=5m ./...
...
run-e2e-tests:
	...
	KUBECONFIG=${kubeconfig} go test -mod=$(MOD) -tags="$(platform),prometheus" -covermode=atomic -buildmode=exe -v ./...
